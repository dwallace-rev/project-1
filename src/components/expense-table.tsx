...
export default function ExpenseTable(props) {
    ...
    useEffect(() => {
        async function getExpenses() {
            ...
            if (viewAll) {
                expenses = await axios.get(`https://dw-proj1-backend.azurewebsites.net/expenses`).then(response => { return response.data });
            }
            else {
                expenses = await (await axios.get(`https://dw-proj1-backend.azurewebsites.net/employeeExpenses/${empId}`)).data
            }
